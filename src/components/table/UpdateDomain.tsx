...
export default function DomainWidgetDialog({
  open,
  setOpen,
  domains,
  data,
...
DialogProps) {
  ...
  const [value, setValue] = React.useState<FormValues>({ domainName: "" });
  ...
  const handleSwitchChange = async (domainId: any) => {
    ...
    setDomains((prevDomains: Domain[]) => {
      return prevDomains.map((domain: any) =>
        domain.id === domainId
          ? { ...domain, domainName: value.domainName } // Update domainName
          : domain
      );
    });
    ...
    try {
      ...
      setDomains((prevDomains: Domain[]) => {
        return prevDomains.map((domain: any) =>
          domain.id === domainId
            ? { ...domain, domainName: domainToUpdate.domainName } // Revert the domainName
            : domain
        );
      });
      setIsLoading(false);
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        >
          New Domain {/* Or make this dynamic */}
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
        ...
        >
          ...
          <TextField
            ...
            onChange={(e) => setValue({ ...value, domainName: e.target.value })}
          />
        </Box>
        ...
        >
          ...
          >
            {isLoading ? "Creating..." : "Change Domain"}
          </Button>
        </Box>
      </BootstrapDialog>
    </React.Fragment>
